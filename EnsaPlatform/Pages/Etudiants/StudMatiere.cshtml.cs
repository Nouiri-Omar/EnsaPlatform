...
{
    public class StudMatiereModel : PageModel
    {
        UserManager<EnsaPlatformUser> currentuser;
        private readonly EnsaPlatform.Data.EnsaContext _context;
        public StudMatiereModel(EnsaPlatform.Data.EnsaContext context, UserManager<EnsaPlatformUser> userManager)
        {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            if (User.IsInRole("student"))
            {
                ...
                Etudiant = await _context.Etudiants
                    .Include(n => n.Niveau)
                    .ThenInclude(mn => mn.Module_Niveaus)
                    .ThenInclude(mo => mo.Module)
                    .ThenInclude(mm => mm.Module_Matieres)
                    .ThenInclude(ma => ma.Matiere)
                    .AsNoTracking()
